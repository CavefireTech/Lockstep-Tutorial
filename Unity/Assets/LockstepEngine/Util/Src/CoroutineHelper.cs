        ...
        public override bool keepWaiting {
            get {
                if ((double) this.m_WaitUntilTime < 0.0)
                    this.m_WaitUntilTime = FTime.realtimeSinceStartup + this.waitTime;
                bool flag = (double) FTime.realtimeSinceStartup < (double) this.m_WaitUntilTime;
                if (!flag)
    ...
    internal class CoroutineRunner {
        ...
        public void SetRoutineInfo(ref RoutineInfo objRoutineInfo){
            if (objRoutineInfo.routine.Current is YieldInstruction) {
                ...
                objRoutineInfo.objYieldInfo = new WaitForSecondsInfo(FTime.timeSinceLevelLoad);
            }
        }
        ...
        public bool DealWithYieldInstruction(RoutineInfo objRoutineInfo){
            if (objRoutineInfo.objYield is WaitForSeconds waitForSec) {
                float objSpan = FTime.timeSinceLevelLoad - ((WaitForSecondsInfo) objRoutineInfo.objYieldInfo).BeginTime;
